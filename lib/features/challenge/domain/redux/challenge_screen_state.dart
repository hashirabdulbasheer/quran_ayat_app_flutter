...
class ChallengeScreenState extends Equatable {
  ...
  List<QuranQuestion> allApprovedAnswers(QuranUser user) {
    return _filterQuestionsBasedOn((element) => element.status == QuranAnswerStatusEnum.approved);
  }

  List<QuranQuestion> _filterQuestionsBasedOn(bool Function(QuranAnswer element) answerCondition) {
    List<QuranQuestion> questions = List.from(allQuestions);
    List<QuranQuestion> filteredQuestions = [];
    for (QuranQuestion question in questions) {
      List<QuranAnswer> answers = question.answers.where((element) => answerCondition(element)).toList();
      // latest on top
    ...
    // latest on top
    filteredQuestions.sort((a, b,) => b.createdOn.compareTo(a.createdOn));
